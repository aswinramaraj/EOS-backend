...
@Injectable()
export class ServiceOrderProposalsService {
  private readonly logger = new Logger(ServiceOrderProposalsService.name);

  constructor(private readonly prisma: PrismaService) {}

  /**
   * POST /service-order-proposals
   *
   ...
   */
  async create(dto: CreateServiceOrderProposalDto) {
    ...
    try {
      return await this.prisma.service_order_proposals.create({
        data: {
      ...
      this.logger.error('DB error while creating service order proposal', err);
  ...
  /**
   * GET /service-order-proposals
   */
  async findAll() {
    try {
      return await this.prisma.service_order_proposals.findMany({
        orderBy: { id: 'asc' },
      });
    } catch (err) {
      this.logger.error('DB error while fetching service order proposals', err);
      throw new InternalServerErrorException({
        message: 'Something went wrong. Please try again.',
        errorCode: 'INTERNAL_ERROR',
      });
  ...
  /**
   * GET /service-order-proposals/:id
   *
   * Error cases:
  ...
  /**
   * PUT/PATCH /service-order-proposals/:id
   *
   ...
   *  404 VENDOR_NOT_FOUND                 – vendor_id does not exist
   */
  async update(id: number, dto: UpdateServiceOrderProposalDto) {
    const proposal = await this.findById(id);
    ...
    try {
      return await this.prisma.service_order_proposals.update({
        where: { id },
        data: {
          vendor_id: dto.vendor_id,
        },
      ...
      this.logger.error('DB error while updating service order proposal', err);
      throw new InternalServerErrorException({
        message: 'Something went wrong. Please try again.',
        errorCode: 'INTERNAL_ERROR',
      });
  ...
  /**
   * PATCH /service-order-proposals/:id/finance-review
   *
  ...
  /**
   * PATCH /service-order-proposals/:id/hod-review
   *
  ...
  /**
   * DELETE /service-order-proposals/:id
   *
   * Error cases:
   *  404 SERVICE_ORDER_PROPOSAL_NOT_FOUND – no proposal with the given id
  ...
  async remove(id: number) {
    ...
    let serviceOrderCount: number;

    try {
      serviceOrderCount = await this.prisma.service_orders.count({
    ...
    try {
      return await this.prisma.service_order_proposals.delete({
        where: { id },
      });
    } catch (err) {
      this.logger.error('DB error while deleting service order proposal', err);
      throw new InternalServerErrorException({
        message: 'Something went wrong. Please try again.',
        errorCode: 'INTERNAL_ERROR',
      });
